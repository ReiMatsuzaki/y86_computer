use super::code::{Code, Register, Dest, ModDest, Imm};
...
impl Scanner {
    ...
    fn error<T>(&self, message: String) -> Result<T, ScanError> {
        Err(ScanError { line_num: self.line_num, message })
    }
    ...
    fn scan_command(&self, input: &str) -> Result<Code, ScanError> {
        ...
        match parts.as_slice() {
            ["halt"] => Ok(Code::Halt),
            ["nop"] => Ok(Code::Nop),
            ["rrmovq", ra, rb] => Ok(Code::Rrmovq(self.scan_register(ra)?, self.scan_register(rb)?)),
            ["irmovq", v, rb] => Ok(Code::Irmovq(self.scan_value(v)?, self.scan_register(rb)?)),
            ["rmmovq", ra, m] => Ok(Code::Rmmovq(self.scan_register(ra)?, self.scan_mod_dest(m)?)),
            ["mrmovq", m, ra] => Ok(Code::Mrmovq(self.scan_mod_dest(m)?, self.scan_register(ra)?)),
            ...
            ["cmove", ra, rb] => Ok(Code::Cmove(self.scan_register(ra)?, self.scan_register(rb)?)),
            ["cmovne", ra, rb] => Ok(Code::Cmovne(self.scan_register(ra)?, self.scan_register(rb)?)),
            ...
            _ => self.error(format!("statement parts mismatched: {0:?}", parts.as_slice()).to_string()),
        }
    }
